...
from dates t1
join (select o.person_id,p.gender, p.birthdate, min(date_format(o.obs_datetime, '%Y-%m')) as enrolled_month, 
min(date(o.obs_datetime)) as enroll_date,
min(if(o.concept_id=160534,o.value_datetime, null)) as ti_date,
min(if(o.concept_id=160540,case o.value_coded
when 5622 then 'other'
...
from dates t1
join (select patient_id,coalesce(active_status.ti_art_startDate, min(start_date))  as art_startDate, p.gender, p.birthdate,
date_format(min(o.start_date), '%Y-%m') as art_start_month,
getWhoStage(patient_id, min(start_date)) as whoStage,
active_status.*
from orders o
