...
where m.encDate between six_month and endDate 
...
where n.lastEncDate between six_month and endDate and (datediff(endDate, dob) / 365.25)>=15 and (status_checked=1 or suspected_concept_filled=1 or diagnosis_done=1 or no_signs_present=1)
) as monthlyCount
from month_dates 
group by year(endDate), month(endDate)
